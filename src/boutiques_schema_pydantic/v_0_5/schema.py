...
class Descriptor(pydantic.BaseModel):
    ...
    deprecated_by_doi: Optional[Union[StringProperty, bool]] = pydantic.Field(
        ...
        description="doi of the tool that deprecates the current one. May be set to "
        "'true' if the current tool is deprecated but no specific tool deprecates it.",
        default=None,
    )
    ...
    environment_variables: Optional[list[properties.EnvironmentVariable]] = (
        pydantic.Field(
            ...
            description="An array of key-value pairs specifying environment variable "
    ...
    tags: Optional[dict[str, Union[str, list[str], bool]]] = pydantic.Field(
        description="A set of key-value pairs specifying tags describing the pipeline. "
        "The tag names are open, they might be more constrained in the future.",
        default=None,
    )
    error_codes: Optional[list[properties.ErrorCode]] = pydantic.Field(
        alias="error-codes",
        description="An array of key-value pairs specifying exit codes and their "
        "description. Can be used for tools to specify the meaning of particular exit "
        "codes. Exit code 0 is assumed to indicate a successful execution.",
        min_length=1,
        default=None,
    )
